...
import { walkForwardValidate } from "@/lib/walkforward";
...
/**
 * GET /api/admin/walkforward?days=730&horizon=10&step=5&trainFrac=0.6&limit=200
 *
 * Walk-forward search over bearish/bullish factor-weight scale multipliers
 * (see lib/walkforward.ts): picks the best scales on the train window of
 * each ticker's history, then reports out-of-sample (test window) Spearman
 * vs. the current (1,1) baseline. Admin-only; CRON_SECRET bearer accepted.
 */
export async function GET(req: Request) {
  ...
  const result = walkForwardValidate(barsMap, horizon, step, trainFrac);
